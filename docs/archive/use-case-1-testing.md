# Use Case 1: Legal Case Analysis — Agent Testing

## Executive Summary

Use Case 1 evaluated 11 agent configurations across 10 prompts (110 total test runs) comparing structured-data agents (MCP/SQL) against document-backed agents (SharePoint and uploaded files in Copilot Studio). All data is synthetic — 50 legal cases, 277 people, 333 timeline events, 338 statements, 151 discrepancies.

**Key findings:**
- **Web SPA (custom chat UI + MCP/SQL)** ranked #1: 10 Pass, 0 Partial, 0 Fail
- **MCP-GCC (Copilot Studio + MCP/SQL)** ranked #2: 9 Pass, 0 Partial, 1 Fail
- **MCP-Com and SP/PDF-Com** tied #3: 8 Pass, 2 Partial, 0 Fail
- Document agents scored between 4-7 Pass, with GCC variants generally trailing Commercial
- GCC Copilot Studio is locked to GPT-4o; Commercial uses GPT-4.1 — this model gap explains many GCC failures

**Danger taxonomy** (discovered through 110 test runs):
1. Critical: False negative — data retrieved but not recognized
2. Critical: Misleading source faithfully reproduced (7/8 document agents on skeletal survey)
3. High: Misattribution — real fact attributed to wrong person (4/8 on 8 PM time)
4. High: Hallucinated fact stated with confidence
5. Medium: Hallucinated case number, silent failure

**Scoring:** Pass = correct, complete, properly sourced. Partial = partially correct or incomplete. Fail = wrong answer, hallucination, or no answer.

Testing dates: March 5-7, 2026 (Rounds 1-8).
